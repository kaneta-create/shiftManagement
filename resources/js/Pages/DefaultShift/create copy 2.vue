        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div>
                        <h1 class="text-center text-3xl mb-4 text-indigo-500">シフト登録</h1>
                    </div>
                    <form @submit.prevent="storeDefaultShift">
                        ...
                        <div class="text-center mb-4">
                            ...
                            <input type="text" id="employee_number" required name="employee_number" v-model="form.employee_number" class="border border-gray-500 p-2 focus:outline-none focus:ring-2 focus:ring-indigo-400">
                        ...
                        <div class="lg:w-2/3 w-full mx-auto overflow-auto">
                            <table class="table-auto w-full text-left whitespace-no-wrap">
                                ...
                                <tbody>
                                <tr v-for="(dayOfWeek, key) in props.day_of_week_name" :key="key">
                                    <td class="border-y-2 border-gray-200 px-4 py-3">{{ dayOfWeek }}</td>
                                    <td class="border-y-2 border-gray-200 px-4 py-3 relative">
                                        <select id="clock_in" v-model="form[dayOfWeek].start_time" class="mb-2">
                                    ...
                                    <td class="border-y-2 border-gray-200 px-4 py-3">
                                        <select id="clock_out" v-model="form[dayOfWeek].end_time" class="mb-2">
                                            <option value="" disabled>-- 選択 --</option>
                                            <option v-for="time in timeOptions" :key="time" :value="time">{{ time }}</option>
                                        </select>
                                    </td>
                        ...
                        <!-- 登録ボタン -->
                        <div class="mt-6">
                            <button id="register-button" dusk="register-button" class="flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">
                                登録
                            </button>
                        </div>
                    </form>
