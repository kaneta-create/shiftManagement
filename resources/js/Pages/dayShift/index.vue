                    ...
                    <div class="p-6 text-gray-900">
                        ...
                        <div class="overflow-x-auto">
                          <table class="table-auto w-full text-left whitespace-no-wrap">
                            <thead>
                            <tr>
                                <th class="py-2 px-2 border-2 title-font tracking-wider font-medium text-white text-sm bg-gray-500 rounded-tl rounded-bl text-nowrap text-center">名前</th>
                                <th class="py-2 px-2 border-2 border-2text-center border-x-1 title-font tracking-wider font-medium text-white text-sm bg-gray-500 text-center">
                                    勤務時間
                                </th>
                                <th v-for="(hour ,index) in props.totalHour" class="py-2 px-2 border-2 border-x-1 title-font tracking-wider font-medium text-white text-sm bg-gray-500 text-center">
                                  <div class="flex">
                            ...
                            <tr v-for="(shift, index) in matchedShifts" :key="index" class="border-2 hover:bg-gray-400 transition duration-150 ease-in-out">
                                <td class="border-t-2 border-gray-200 p-left-4 py-2 bg-gray-500 text-white text-center text-sm text-nowrap">
                                    {{ shift.employeeName }}
                                </td>
                                <td class="border-t-2 border-gray-200 p-left-4 py-2 text-center text-sm bg-gray-100">
                                    {{ shift.clockInHour }}:{{ shift.clockInMinutes }}~{{  shift.clockOutHour === '00' ? '24' : shift.clockOutHour  }}:{{ shift.clockOutMinutes }}
                                </td>
                                <td  v-for="(hour ,index) in props.totalHour" :style="getShiftClass(hour, shift)" class="border-2 bg-gray-300 border-gray-200 px-4 py-2">
                                </td>
                            </tr>
